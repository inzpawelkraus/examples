...
class Invoices_model extends Crud_model {
    ...
    function get_max_number($month, $year) {
        $invoices_table = $this->db->dbprefix('invoices');
        $sql = "SELECT COUNT(id)+1 as number FROM $invoices_table WHERE MONTH(`bill_date`) = '" . $month . "'  AND YEAR(`bill_date`) = '" . $year . "'";
        $maxnumber = $this->db->query($sql)->result();
        return $maxnumber[0]->number;
    }
