...
class Invoice_payments extends MY_Controller {
    ...
    private function _make_payment_row($data) {
        ...
        if ($this->login_user->user_type == "staff") {
            $invoice_url = anchor(get_uri("invoices/view/" . $data->invoice_id), get_invoice_id($data->number));
        } else {
            $invoice_url = anchor(get_uri("invoices/preview/" . $data->invoice_id), get_invoice_id($data->number));
        }
        return array(
            $invoice_url,
            ...
            modal_anchor(get_uri("invoice_payments/payment_modal_form"), "<i class='fas fa-pencil-alt'></i>", array("class" => "edit", "title" => lang('edit_payment'), "data-post-id" => $data->id))
