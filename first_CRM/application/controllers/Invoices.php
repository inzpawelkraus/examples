...
class Invoices extends MY_Controller {
    ...
    function save() {
        ...
        $realization_date = $this->input->post('invoice_realization_date');
        ...
        if(!$id) {
            ...
            $year = date("Y");
            $date = date("m/Y");
            $number = $this->Invoices_model->get_max_number($month, $year) . "/" . $date;
    ...
    private function _make_row($data, $custom_fields) {
        $invoice_url = "";
        if ($this->login_user->user_type == "staff") {
            $invoice_url = anchor(get_uri("invoices/view/" . $data->id), "#" . $data->number);
        } else {
            $invoice_url = anchor(get_uri("invoices/preview/" . $data->id), "#" . $data->number);
        }
        ...
        $row_data[] = modal_anchor(get_uri("invoices/modal_form"), "<i class='fas fa-pencil-alt'></i>", array("class" => "edit", "title" => lang('edit_invoice'), "data-post-id" => $data->id))
    ...
    private function _make_recurring_row($data) {
        ...
        return array(
            ...
            to_currency($data->invoice_value, $data->currency_symbol),
            modal_anchor(get_uri("invoices/modal_form"), "<i class='fas fa-pencil-alt'></i>", array("class" => "edit", "title" => lang('edit_invoice'), "data-post-id" => $data->id))
            . js_anchor("<i class='fa fa-times fa-fw'></i>", array('title' => lang('delete_invoice'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("invoices/delete"), "data-action" => "delete"))
        );
    }
    ...
    private function _make_item_row($data) {
        ...
        return array(
            ...
            modal_anchor(get_uri("invoices/item_modal_form"), "<i class='fas fa-pencil-alt'></i>", array("class" => "edit", "title" => lang('edit_invoice'), "data-post-id" => $data->id))
    ...
    function send_invoice_modal_form($invoice_id) {
        ...
        if ($invoice_id) {
            ...
            $parser_data["INVOICE_NUMBER"] = $invoice_info->number;
