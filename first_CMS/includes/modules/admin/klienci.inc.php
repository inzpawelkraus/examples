...
$oUsers->CheckPrivileges('aktualnosci_administration');
...
$modul = 'aktualnosci'; // nazwa modulu, tabela w bazie, link w adresie
...
// dodajemy do bazy
} elseif ($_POST['action'] == 'Dodaj podstronę') {
...
function showAdd() {
    ...
    $tpl->setPageTitle('Nowy wpis');
    $tpl->assign('menu_selected', 'new_' . $modul);
...
function showEdit($id, $zakladka = 'glowna') {
    ...
    if ($article = $oAktualnosci->LoadArticleById($id)) {
        ...
        $tpl->assign('galleries', $oGallery->LoadGalleriesNames());
        $tpl->setPageTitle('Edycja artykułu');
...
function showArticles() {
    ...
    $tpl->setPageTitle('Zarządzanie artykułami');
    $tpl->assign('menu_selected', $modul);
